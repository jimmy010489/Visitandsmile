...
export async function agentComptable(state) {
    ...
    try {
        switch (action) {
            ...
            case 'enregistrer_vente': {
                ...
                await supabase.from('activity_feed').insert({
                    ...
                    agent:   'compta',
                    icon:    'fa-euro-sign',
                    message: `<strong>Agent Comptable</strong> — Vente enregistrée : ${data.property_name}`,
                });
            ...
            case 'generer_rapport': {
                ...
                const response = await model.invoke([{
                    role:    'user',
                    content: `Tu es l'agent comptable de Visit & Smile. Génère un rapport financier concis à partir de ces données : ${JSON.stringify(stats)}. Inclus : CA du mois, URSSAF dues, net, et 2 recommandations. Max 150 mots.`
                }]);

                const reportText = response.content;
                const usage      = response.usage_metadata;

                // Logger les tokens réels
                ...
                // Sauvegarder dans l'activity feed
                await supabase.from('activity_feed').insert({
                    user_id: userId,
                    agent:   'compta',
                    icon:    'fa-chart-bar',
                    message: `<strong>Rapport financier</strong> — ${reportText.substring(0, 100)}...`,
                });
            ...
            case 'rappel_urssaf': {
                await supabase.from('activity_feed').insert({
                    ...
                    message: '<strong>Rappel URSSAF</strong> — Déclaration à venir dans moins de 7 jours',
                });
        ...
        return {
            results: { ...state.results, agent_comptable: { error: err.message } },
            errors:  [{ agent: 'comptable', error: err.message }],
        };
    }

    return {
        results: { ...state.results, agent_comptable: { action, result } },
    };
}

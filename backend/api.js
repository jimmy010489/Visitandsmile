...
const globalLimiter = rateLimit({
    ...
    max: 60,                // 60 requêtes max / minute / IP
...
const dispatchLimiter = rateLimit({
    windowMs: 60 * 1000,
    max: 15,                // 15 dispatches max / minute / IP
    standardHeaders: true,
    legacyHeaders: false,
    message: { error: 'Limite d\'appels IA atteinte. Réessayez dans un instant.' },
});
 ...
 */
app.post('/api/graph/dispatch', dispatchLimiter, authMiddleware, async (req, res) => {
    try {
...
app.get('/api/graph/state/:threadId', authMiddleware, async (req, res) => {
    try {
        ...
        console.error('[API] GetState error:', err);
